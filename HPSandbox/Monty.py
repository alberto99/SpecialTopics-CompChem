...
class Monty:
	...
	self.g = random.Random(config.randseed+config.REPLICATEMPS.index(temp))               # NOTE:  random.Random is an object that
...
"""
    ...
    def metropolis(self,replica):
        ...
        #print "METROPOLIS***********"
        E = self.energy(replica.chain)
        Es= self.restraint.energy(replica.chain)
        ...
        #print E,Es,E+Es,thisenergy,self.lastenergy,boltzfactor
        #print "Before: ",replica.chain.coords
        #print "After:  ",replica.chain.nextcoords
...
class DistRestraint:
	...
	"""Return the sum of squared-distances over the selected contacts."""
        ...
        for i in range(0,len(self.contacts)):
            ...
            #we don't want penalty if we are stablishing the contact that we are restraining
            D = D -1 
        return D
