...
class Config:
  ...
  def __init__(self, filename=None):
    ...
    self.epsilon = self.eps*self.k*self.T   # energetic strength of each contact -- in units Joules (J/mol)
                                            # kT = ~8.314 J/mol. The energy of a contact is the same at all temperatures, but will have a different boltzmann
                                            #weight depending on which replica it is in

    self.POOL_SPRINGS = [(1,4),(6,9)]       # Possible springs for each replica to have
    self.ACTIVE_SPRINGS = 2                 # number of springs that should be active at any replica
  ...
  def read_configfile( self,filename ):
    ...
    for i in range(0,len(lines)):
      ...
      if len(fields)>0:
        ...
        if fields[0] == 'ACTIVE_SPRINGS':
           self.ACTIVE_SPRINGS = eval(string.joinfields(fields[1:]))
    
        if fields[0] == 'POOL_SPRINGS':
           self.POOL_SPRINGS = eval(string.joinfields(fields[1:]))
  ...
  def print_config(self):
    ...
    print('%-30s %s'%('ACTIVE_SPRINGS',repr(self.ACTIVE_SPRINGS)))
    print('%-30s %s'%('POOL_SPRINGS',repr(self.POOL_SPRINGS)))
